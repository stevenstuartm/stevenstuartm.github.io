...
date: 2026-03-09
description: "Production databases should do one thing well: serve transactions and production queries. Reporting pressure gradually distorts schemas until they serve two masters and compromise for both. Separating those concerns early is what keeps the production model honest."
tags: [architecture, databases, design-patterns, distributed-systems, data-modeling, event-sourcing, cqrs]
---

Production databases should do one thing well: serve production transactions and production queries. In practice, most don't. Their schemas are shaped as much by reporting needs as by the domain they were built to serve, and the distortion compounds over time until the schema serves two masters and compromises for both.


The transactional model and the reporting model have different shapes, different consumers, different evolution pressures, and different performance profiles. A transactional schema optimizes for write consistency, referential integrity, and the access patterns of the application that owns it. A reporting schema optimizes for read throughput, aggregation, and the access patterns of analysts and dashboards. Consider what happens when the analytics team asks for a denormalized `order_summary` view on the production database so their dashboards load faster. The DBA obliges, adds a materialized view, and now every schema migration has to account for it. Six months later the team wants to split the `orders` table into `orders` and `order_line_items`, but the view is embedded in 40 dashboard queries and a nightly export job. The refactor stalls, and the production schema fossilizes around a reporting concern.

Every schema decision becomes a negotiation between these competing concerns, and reporting usually wins because it's the most visible to leadership and the most painful to change after the fact. The question isn't which concern to prioritize. It's whether those concerns can be separated so each model serves the workload it was designed for. The separation can be reactive, tapping into what the database already captures, or intentional, making the application responsible for producing reporting-quality records in the write path.

Not every system needs this separation. A small team with a single database, low reporting complexity, and a schema that's still fluid can query production directly without meaningful friction. The distortion described here is a scaling problem: it emerges when reporting consumers multiply, when dashboards become load-bearing, and when schema changes require cross-team coordination. Recognizing when you've crossed that threshold is more useful than separating preemptively.
...
The approach breaks down when reporting needs diverge from the production schema's shape. Reporting consumers write increasingly complex queries with multiple joins, or they start requesting schema changes to production to make their queries simpler, which is exactly the distortion this post is about. The replica also can't capture history. It mirrors current state, so if a record changes twice between queries the intermediate state is gone. Once reporting needs outgrow what direct queries against the production schema can serve, you're in ETL territory.
...
Both production and reporting deserve to be first-class concerns, but recognizing that from the start doesn't mean designing one big schema that tries to serve both. It means decoupling from the schema entirely and fully realizing the power of the data you already have. Production databases optimize for their inserts and their queries. Dev teams deploy and evolve a component's database as needs are discovered, without asking permission. Reporting teams get richer, more contextual insights that are readily available instead of painfully reconstructed from schemas that were never designed for them. And the two groups stop being at each other's throats, because they're no longer competing for the same resource.
